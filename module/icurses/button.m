...
IcButton: module
{
	...
	#   "Label" -> "> Label <"
	#
	press: fn(u: ref IcUi->Ui, id: string, ms: int): int;

	#
	# Temporarily redraw a button-like node using an explicit pressed label.
	#
	presslabel: fn(u: ref IcUi->Ui, id, pressed: string, ms: int): int;
	...
	#
	click: fn(u: ref IcUi->Ui, id: string): IcMsg->Msg;

	#
	# Visual press followed by click dispatch.
	#
	activate: fn(u: ref IcUi->Ui, id: string, ms: int): IcMsg->Msg;

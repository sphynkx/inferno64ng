...
Каждый элемент имеет уникальный целочисленный идентификатор. По этому идентификатору к элементу можно обращаться при построении интерфейса, изменении состояния, обработке сообщений, управлении фокусом и перерисовке. Идентификатор также используется как адрес в сообщениях между элементами и приложением. Приложение задает и хранит эти ID как `int`-значения и передает их в последующие вызовы API.
...
`shadowwindow()` создает два узла: узел тени и узел окна. Поэтому в новой `int`-модели ему передаются два ID: `MainShadowId` и `MainWinId`.
...
Когда достаточно поменять bounds, а когда нужно пересоздать UI. При полной перестройке нужно заново построить дерево и заново создать все узлы с корректными `int` ID.

### <a id="6-6">6.6</a>. Canvas при resize [&uarr;](#0)

Особенности canvas-backed интерфейсов. Canvas также адресуется через `int` ID, поэтому после rebuild нужно заново создать canvas-узел и заново заполнить его содержимое.
...
Когда использовать canvas и чем он отличается от обычных элементов. Canvas-операции `canvasclear`, `canvasfill`, `canvasputc` и `canvasputs` принимают `int` ID canvas-узла.
...
Высокоуровневая модель, где приложение получает готовые шаги событий. В `Step.msg` поля `src` и `dst` являются `int`, а `cmd` остается строкой.
...
Отдельная типовая ошибка после перехода на `int` ID — использование идентификатора узла, который не был создан в текущем дереве. Если приложение создало новый `Ui` и заново построило дерево, оно должно заново создать все нужные узлы.
